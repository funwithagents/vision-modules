...
_PKG = _REPO_ROOT / "src" / "vision_modules"
_SPECS_DIR = _REPO_ROOT / "specs"

# Matches link targets like `src/vision_modules/foo.py` anywhere in AGENTS.md.
_MODULE_LINK = re.compile(r"src/vision_modules/([A-Za-z_][A-Za-z0-9_]*\.py)")
...
def test_every_concept_module_is_governed_by_a_spec():
    ...
    for spec in _spec_files():
        for rel in _parse_frontmatter(spec).get("code", []):
            if rel.startswith("src/vision_modules/"):
