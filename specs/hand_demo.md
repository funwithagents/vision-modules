...
The demo measures and displays the *actual* rate of each stage, right under the sliders, e.g. `Hand stage: 24.3 fps (target 30) · Classifier: 4.8 fps (target 5)` — the "target" side reads `hands.target_fps`/`gestures.target_fps` live, so it tracks the fps sliders above rather than the CLI's original value:
...
- One `FpsMeter` per stage (`hands`, `gestures`); each samples that stage's `published_count` directly — no need to touch `.latest()` at all for this.
