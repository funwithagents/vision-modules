...
3. **One `gr.Row` of columns** — the two images at `scale=2, min_width=240` each, then one classification column per slot at `scale=1, min_width=120` each, side by side (with the default `max_hands=2`: `2 · 2 · 1 · 1`). The lowered `min_width`s matter: Gradio's default of 320px per column wraps the last column onto a second row at common laptop widths, whatever the scales:
   - **Input** — `gr.Image(sources=["webcam"], streaming=True)`, the raw browser feed, untouched.
   - **Detected** — a second, output-only `gr.Image`: a copy of the same frame with one rectangle per published hand drawn on it (`HandResult.hands[i].bbox`), in a **per-slot colour** (`SLOT_COLORS`, a short fixed BGR palette indexed by `i`, green first) with the slot number `i + 1` as a small tag at the box's top-left corner. That tag is the only text on the image: it is what links a box to its panel, and it stays readable because it is one digit, not a label. No classification text goes on the image.
   - **Classifications** — **one `gr.Label` per slot, each in its own `scale=1` column** next to the images, `max_hands` of them, labelled `Hand 1`, `Hand 2`, … in the same slot colours' names (`Hand 1 (green)`), each showing that slot's **three best** classes (`num_top_classes=3` — `summarize` still returns the full `HandGesture.scores`, the component trims the display, which keeps the panel short enough for a narrow column). `gr.Label` sorts by score, so the top class is always the top row; when it also cleared `threshold` (`HandGesture.label` is not `None`), its key is prefixed with a checkmark so the validated call is visually distinguished from "just the highest score" without a second component. Placeholder single-entry dicts (`{"no hand": 1.0}`, `{"...": 1.0}`) stand in for "no hand in this slot" / "hand present, classifier hasn't caught up". A slot's panel tracks a physical hand across frames because [hand.md](hand.md)'s selection is slot-stable — two hands do not swap panels when their sizes cross.

`summarize(hr, gesture, index)` is the per-slot pure helper (`index` selects `hands[index]` on both results, out of range meaning "no hand"), and `on_frame` returns one dict per panel: `outputs=[cam_out, *score_panels]`.

`cam_in.stream(fn=on_frame, inputs=cam_in, outputs=[cam_out, *score_panels], stream_every=1 / args.hand_fps)` drives the loop; `on_frame` reads the raw frame from `cam_in` and returns `(annotated_frame, score_dict_1, …, score_dict_max_hands)` for the other components. `stream_every` matters: it caps how often the browser is even allowed to hand the server a frame, independent of every `target_fps` downstream, and Gradio's own default (`0.5`, i.e. 2 fps) would silently bottleneck the whole pipeline below `--hand-fps` regardless of its value — so it's set from `--hand-fps` instead of left at the default.
...
`--mirror`, `--hand-fps` (default 30), `--classifier-fps` (default 5), `--threshold` (default 0.55, the slider's starting value), `--max-hands` (default 2, `>= 1`: how many hands are detected, published, classified and given a panel — [hand.md](hand.md)'s single `max_hands`) — passed straight through to `StreamProvider`/`HandStage`/`GestureClassifier`. Defaults match [hand.md](hand.md) and [gesture_classifier.md](gesture_classifier.md). `--max-hands` is fixed at launch, not a live slider: it sets the number of panels, which is layout.
...
The demo is an application, so it has no live test of its own, but its pure pieces — `PushFrameSource`, `FpsMeter`, `summarize` (per slot), `draw_boxes` (per-slot colour and tag), `snapshot_path`, `save_snapshot` (driven through a real `HandStage` / `GestureClassifier` with scripted backends) — are unit-tested in `tests/test_hand_demo.py` (the `examples/` directory is on pytest's `pythonpath` and in pyright's scope, see [project.md](project.md)). The pipeline wiring it uses is covered end to end by `tests-e2e/test_hand_pipeline_live.py`.
...
3. ~~**Multi-hand classification display.**~~ Closed: one panel per slot, colour-linked to its box (see Layout). What remains open is only presentation at large `--max-hands` (one column per panel in the same row squeezes the images and the panels); wrapping panels onto a second row or a compact table would be the next step if anyone runs the demo with more than three or four hands.
