...
A live test that needs real credentials must **skip — never fail** — when they're absent, so you exercise only the services you hold keys for and a contributor (or CI) with none is never broken. `tests-e2e/support.require_env(NAME)` implements this: it returns the env var or calls `pytest.skip(...)` when it's unset. Credentials come from the environment, never committed.

Not every live test is gated on a credential: the model-backed tests (MediaPipe, the HaGRID ViT) need only network access on their first run, to download weights that are cached afterwards. Those **fail, not skip**, when the download fails — there is no key to be missing, and silently skipping would hide a broken model path. Run the live tier with network the first time.
...
- **`pyright` covers test code and examples too:** its `include` is `src`, `tests`, `tests-e2e` and `examples`, so tests and the demo are type-checked alongside the library rather than being a blind spot.
