...
Stage[TIn, TOut](upstream, target_fps: float | None, name: str | None = None)
  ...
  .latest() -> TOut | None
  .last_input -> TIn | None   # the item most recently handed to process(); None until the first; carries over restarts
...
Worker loop: `item = upstream.latest()`; if `item` is `None` or its `frame_id` equals the last processed one, wait; else record `item` as `last_input`, `out = self.process(item)`, publish `out` when not `None` and increment `published_count`, record the `frame_id`; then sleep so iterations respect `target_fps` (`None` = as fast as upstream delivers). Timing uses `time.monotonic()`.
...
- **`last_input` is what this stage last looked at.** It is the exact item handed to `process()`, kept as a reference in a `LatestValue` slot (no copy — published values are immutable and never reused, [stream.md](stream.md)), readable from any thread. It answers "what did this module consume?" without racing the upstream: the upstream's own `latest()` has usually moved on by the time a caller looks (a 5 fps module under a 30 fps stage sees one item in six), whereas `last_input` is pinned to this stage's own cadence. It is set **before** `process()` runs, so while `process()` is failing it names the offending input (pair it with `last_error`). Because it carries `frame_id`, `last_input.frame_id == latest().frame_id` tells a caller whether the published output was computed from it. This is the hook [snapshot.md](snapshot.md) builds `save_input` on.
